...
DELETE_PREFIX = os.environ.get('DELETE_PREFIX', 'p012/')  # Prefixo(s) da(s) pasta(s) a ser(em) deletada(s) - pode ser múltiplos separados por vírgula
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Normalizar o prefixo (garantir que termine com /)
        # Cada Lambda processa apenas uma pasta específica
        if not DELETE_PREFIX:
            logger.warning("DELETE_PREFIX não está configurado. Nenhum arquivo será deletado.")
            return {
                'statusCode': 400,
                'body': json.dumps({
                    'error': 'Prefixo não configurado',
                    'message': 'DELETE_PREFIX deve ser configurado (ex: p012/adc/)'
                })
            }
        
        # Normalizar o prefixo (garantir que termine com /)
        prefix = DELETE_PREFIX.rstrip('/') + '/'
        
        logger.info(f"Iniciando limpeza da pasta '{prefix}' no bucket {SOURCE_BUCKET}")
        
        # Apagar arquivos do bucket de origem
        deleted_count = delete_files_from_bucket(SOURCE_BUCKET, prefix)
        
        logger.info(f"Limpeza concluída. {deleted_count} objetos deletados da pasta '{prefix}' no bucket {SOURCE_BUCKET}")
...
def delete_files_from_bucket(bucket_name: str, prefix: str) -> int:
    ...
    Args:
        ...
        prefix: Prefixo da pasta para filtrar arquivos (ex: 'p012/')
    ...
    try:
        ...
        # Se havia prefixo, também deletar versões desse prefixo (se versionamento estiver habilitado)
        if prefix:
            try:
                versions_paginator = s3_client.get_paginator('list_object_versions')
                versions_pages = versions_paginator.paginate(Bucket=bucket_name, Prefix=prefix)
